...
all_wind<-rbind(pfin,penv,balance) %>% 
...
all_aqua_sf<-rbind(af_pfin,af_penv,af_balance) %>% 
...
all_aqua_sw<-rbind(as_pfin,as_penv,as_balance) %>% 
...
all_wind_env<-rbind(pfin_env,penv_env,balance_env) %>% 
  mutate(indstury = "Wind",
...
all_aqua_sf_env<-rbind(af_pfin_env,af_penv_env,af_balance_env) %>% 
  mutate(indstury = "Fish",
         priority = "Environmental suitability")
...
all_aqua_sw_env<-rbind(as_pfin_env,as_penv_env,as_balance_env) %>% 
  mutate(indstury = "Seaweed",
         priority = "Environmental suitability")
...
```{r}
ggplot(all, aes(x = select, y = cumsum_perc, colour = scenario)) +
  geom_line() +
  theme_bw() + 
  labs(y = "Cumulative",
       x = "No. cells",
